...
Write-Verbose "`tGet-EC2VolumeSummary - START"
...
foreach ($region in $Regions) {
    ...
    $tags = $Null
    if($Profile) {
        Write-Verbose "---`taws ec2 describe-volumes --region $region --profile $Profile --output json"
        ...
        Write-Verbose "---`taws ec2 describe-volumes --region $region --output json"
...
Write-Verbose "`tGet-EC2VolumeSummary - END"
return $volTable
# END
